...
const getWorkoutDates = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '운동 기록 날짜를 가져오는 데 실패했습니다.' });
  }
};
...
const getComments = async (req, res) => {
  try {
    ...
    const comments = await WorkoutLogComment.find({ workoutUserId }).populate('writerUserId', 'username avatar');
...
const createComment = async (req, res) => {
  try {
    ...
    const newComment = new WorkoutLogComment({ workoutUserId, writerUserId, content });
    await newComment.save();

    // populate는 save 이후에 
    const populatedComment = await newComment.populate('writerUserId', 'username avatar');
...
const deleteComment = async (req, res) => {
  try {
    ...
    if (comment.writerUserId.toString() !== currentUserId && workoutOwnerId !== currentUserId) {
